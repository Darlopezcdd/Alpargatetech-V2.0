    ...
    <div class="max-w-md w-full bg-brand-white/95 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden z-10 animate-fade-in-up border border-white/20 relative">
        ...
        <div class="px-8 py-10 md:px-10">
            ...
            <div class="text-center mb-8">
                @if(!empty($imgLogo))
                    <img src="{{ $imgLogo }}" alt="Logo" class="h-20 w-auto mx-auto mb-4 object-contain drop-shadow-xl hover:scale-105 transition-transform duration-500">
                @endif
            ...
            @if(session('status'))
                <div class="mb-6 bg-green-50 border-l-4 border-brand-gold text-green-800 p-4 rounded text-sm shadow-sm" role="alert">
                    <p>{{ session('status') }}</p>
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6" id="loginForm">
                ...
                <div class="group">
                    <label for="email" class="block text-xs font-bold text-brand-gray uppercase tracking-wider mb-2">Correo Electrónico</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="h-5 w-5 text-gray-400 group-hover:text-brand-gold transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" />
                            </svg>
                        </div>
                        <input id="email" name="email" type="email" autocomplete="email" required value="{{ old('email') }}"
                            class="appearance-none block w-full pl-10 pr-4 py-3 border {{ $errors->has('email') ? 'border-brand-red bg-red-50' : 'border-gray-200 bg-gray-50' }} rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-gold/50 focus:border-brand-gold focus:bg-white text-brand-dark transition-all duration-300 sm:text-sm shadow-inner" placeholder="ejemplo@correo.com">
                    </div>
                </div>

                <div class="group">
                    <label for="password" class="block text-xs font-bold text-brand-gray uppercase tracking-wider mb-2">Contraseña</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="h-5 w-5 text-gray-400 group-hover:text-brand-gold transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <input id="password" name="password" type="password" autocomplete="current-password" required
                            class="appearance-none block w-full pl-10 pr-10 py-3 border {{ $errors->has('password') ? 'border-brand-red bg-red-50' : 'border-gray-200 bg-gray-50' }} rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-gold/50 focus:border-brand-gold focus:bg-white text-brand-dark transition-all duration-300 sm:text-sm shadow-inner" placeholder="••••••••">
                        
                        <button type="button" onclick="togglePassword()" class="absolute inset-y-0 right-0 px-3 flex items-center text-gray-400 hover:text-brand-red focus:outline-none transition-colors duration-200">
                            ...
                            <svg id="eye-off-icon" class="h-5 w-5 hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.542-7a10.05 10.05 0 011.574-2.59M5.37 5.37L18.63 18.63M9.88 9.88a3 3 0 104.24 4.24" />
                            </svg>
                        </button>
                    </div>
                </div>
        ...
        <div class="bg-gray-50 py-4 border-t border-gray-100 text-center">
             <span class="text-xs text-brand-gray/60 font-light">© {{ date('Y') }} Alpargate 3. Todos los derechos reservados.</span>
        </div>
    </div>
