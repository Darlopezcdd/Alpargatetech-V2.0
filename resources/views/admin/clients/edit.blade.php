...
<div class="animate-fade-in-up max-w-2xl">
    <nav class="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span class="text-brand-dark font-medium">Editar: {{ $client->name }}</span>
    </nav>
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-50">
            <h1 class="text-xl font-serif font-bold text-brand-dark">Editar Cliente</h1>
            <p class="text-sm text-gray-400 mt-0.5">Actualizando información de <strong>{{ $client->name }}</strong></p>
        </div>
        <form action="{{ route('admin.clients.update', $client) }}" method="POST" class="p-6 space-y-5">
            @csrf @method('PUT')
            <div>
