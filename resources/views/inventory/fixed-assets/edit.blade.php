...
<div class="animate-fade-in-up max-w-2xl">
    <nav class="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span class="text-brand-dark font-medium">Editar: {{ $fixedAsset->name }}</span>
    </nav>

    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-50">
            <h1 class="text-xl font-serif font-bold text-brand-dark">Editar Activo Fijo</h1>
            <p class="text-sm text-gray-400 mt-0.5">Modificando: <strong>{{ $fixedAsset->name }}</strong></p>
        </div>

        <form action="{{ route('admin.fixed-assets.update', $fixedAsset) }}" method="POST" class="p-6 space-y-5">
            @csrf @method('PUT')

            <div>
                <label for="name" class="block text-sm font-semibold text-brand-dark mb-1.5">
                    Nombre del Activo <span class="text-brand-red">*</span>
                </label>
                <input type="text" id="name" name="name" value="{{ old('name', $fixedAsset->name) }}"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm font-medium resize-none
                                 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition-all">{{ old('description', $fixedAsset->description) }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="quantity" class="block text-sm font-semibold text-brand-dark mb-1.5">
                        Cantidad <span class="text-brand-red">*</span>
                    </label>
                    <input type="number" id="quantity" name="quantity"
                           value="{{ old('quantity', $fixedAsset->quantity) }}" min="0"
                           class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm font-medium
                ...
                <div>
                    <label for="status" class="block text-sm font-semibold text-brand-dark mb-1.5">
                        Estado <span class="text-brand-red">*</span>
                    </label>
                    <select id="status" name="status"
                        ...
                        @foreach(['Operativo', 'En reparación', 'Dado de baja'] as $s)
                            <option value="{{ $s }}" {{ old('status', $fixedAsset->status) === $s ? 'selected' : '' }}>
                                {{ $s }}
                            </option>
                        @endforeach
                    </select>
