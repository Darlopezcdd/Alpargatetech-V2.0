...
@section('content')
    <div class="md:flex md:items-center md:justify-between mb-8">
        <div class="flex-1 min-w-0">
            ...
            <p class="mt-2 text-sm text-brand-gray">Modifique la información del usuario seleccionado.</p>
        </div>
            ...
            <div class="px-8 py-8 sm:p-12">
                ...
                <form action="{{ route('users.update', $user) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-y-8 gap-x-8 sm:grid-cols-6">
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <input type="text" name="name" id="name" autocomplete="name" required
                                ...
                                value="{{ old('name', $user->name) }}" placeholder="Nombre y Apellido">
                            @error('name') <p class="mt-2 text-sm text-brand-red font-medium flex items-center gap-1"><svg
                        ...
                        <div class="sm:col-span-4">
                            <label for="email" class="block text-sm font-bold text-brand-gray mb-1">Correo
                                Electrónico</label>
                            <input type="email" name="email" id="email" autocomplete="email" required
                                class="block w-full rounded-xl border-gray-300 bg-gray-50 px-4 py-3 text-brand-dark placeholder-gray-400 shadow-sm focus:border-brand-gold focus:bg-white focus:ring-2 focus:ring-brand-gold/50 hover:bg-white hover:border-brand-gold/50 transition-all duration-300 sm:text-sm"
                                value="{{ old('email', $user->email) }}" placeholder="ejemplo@correo.com">
                            @error('email') <p class="mt-2 text-sm text-brand-red font-medium flex items-center gap-1"><svg
                                class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>{{ $message }}</p> @enderror
                        ...
                        <div class="sm:col-span-2">
                            ...
                            @error('role') <p class="mt-2 text-sm text-brand-red font-medium flex items-center gap-1"><svg
                                class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>{{ $message }}</p> @enderror
                        ...
                        <div class="sm:col-span-6 border-b border-gray-100 pb-2 mb-2 mt-6 flex items-center gap-2">
                            <div class="h-6 w-1 bg-brand-gold rounded-full"></div>
                            <h3 class="text-xl font-serif font-bold text-brand-dark">Seguridad (Opcional)</h3>
                        </div>

                        <div class="sm:col-span-3">
                            <label for="password" class="block text-sm font-bold text-brand-gray mb-1">Nueva
                                Contraseña</label>
                            <div class="relative">
                                ...
                                <button type="button" onclick="togglePassword('password', 'eye-1', 'eye-off-1')"
                                    ...
                                    <svg id="eye-off-1" class="h-5 w-5 hidden" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @error('password') <p class="mt-2 text-sm text-brand-red font-medium flex items-center gap-1">
                        ...
                        <div class="sm:col-span-3">
                            <label for="password_confirmation"
                                class="block text-sm font-bold text-brand-gray mb-1">Confirmar Nueva Contraseña</label>
                            <div class="relative">
                                <input type="password" name="password_confirmation" id="password_confirmation"
                                    class="block w-full rounded-xl border-gray-300 bg-gray-50 px-4 py-3 pr-10 text-brand-dark placeholder-gray-400 shadow-sm focus:border-brand-gold focus:bg-white focus:ring-2 focus:ring-brand-gold/50 hover:bg-white hover:border-brand-gold/50 transition-all duration-300 sm:text-sm"
                                    placeholder="Repetir nueva contraseña">
                                <button type="button"
                                    onclick="togglePassword('password_confirmation', 'eye-2', 'eye-off-2')"
                                    class="absolute inset-y-0 right-0 flex items-center px-3 text-gray-400 hover:text-brand-gold focus:outline-none transition-colors">
                                    ...
                                    <svg id="eye-off-2" class="h-5 w-5 hidden" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.542-7a10.05 10.05 0 011.574-2.59M5.37 5.37L18.63 18.63M9.88 9.88a3 3 0 104.24 4.24" />
                                    </svg>
                    ...
                    <div class="mt-12 pt-6 border-t border-gray-100 flex justify-end gap-4">
                        ...
                        <button type="submit"
                            class="inline-flex justify-center px-8 py-3 border border-transparent shadow-lg text-sm font-bold rounded-full text-white bg-brand-gold hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-gold transform hover:-translate-y-1 transition-all duration-300 group">
