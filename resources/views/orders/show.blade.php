...
@section('content')
    <div id="main-order-container"
        class="container mx-auto px-4 py-6 font-sans text-brand-dark lg:h-[calc(100vh-80px)] h-auto">
        ...
        <div class="flex flex-col xl:flex-row justify-between items-end mb-6 border-b border-brand-gold/20 pb-4 gap-4">
            <div class="flex items-center gap-4 w-full xl:w-auto">
                ...
                <div>
                    <h6 class="text-brand-gray font-bold tracking-[0.2em] uppercase text-xs mb-1">Gestión de Pedido
                        #{{ $order->id }}</h6>
                    <h1 class="font-serif text-3xl md:text-4xl font-bold text-brand-dark">Mesa {{ $order->mesa->number }}
                    </h1>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                @if($order->status === \App\Enums\OrderStatus::ANOTADO)
                    <div
                        class="bg-yellow-100 text-yellow-800 px-4 py-2 rounded-xl border border-yellow-200 shadow-sm flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full bg-yellow-500 animate-pulse"></span>
                        <span class="text-xs font-bold uppercase tracking-widest">Anotado</span>
                    </div>
                @else
                    <div
                        class="bg-blue-100 text-blue-800 px-4 py-2 rounded-xl border border-blue-200 shadow-sm flex items-center gap-2">
                ...
                <div
                    class="hidden md:flex bg-white border border-brand-gold/20 px-4 py-2 rounded-xl shadow-sm items-center gap-3">
                    ...
                    <div class="text-right">
                        <p class="text-[8px] font-bold text-brand-gray/60 uppercase tracking-widest leading-none mb-1">Fecha
                        </p>
                        <p id="live-date"
                            class="text-[10px] font-bold text-brand-dark uppercase tracking-wide leading-none">
                ...
                <div
                    class="hidden md:flex bg-white border border-brand-gold/20 px-4 py-2 rounded-xl shadow-sm items-center gap-3 min-w-[110px]">
                    ...
                    <div class="text-right">
                        <p class="text-[8px] font-bold text-brand-gray/60 uppercase tracking-widest leading-none mb-1">Hora
                        </p>
        ...
        <div class="flex flex-col lg:flex-row gap-6 lg:h-full pb-10 lg:pb-20">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($category->products as $product)
                                <form action="{{ route('orders.add-product', $order->id) }}" method="POST"
                                    class="ajax-add-product h-full">
                                    @csrf
                                    <input type="hidden" name="product_id" value="{{ $product->id }}">
                                    <button type="submit"
                                        class="w-full h-full group bg-white p-4 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-brand-gold/30 transition-all duration-200 cursor-pointer active:scale-95 flex justify-between items-center relative overflow-hidden text-left">
                                        <div
                                            class="absolute inset-0 bg-brand-gold/5 opacity-0 group-active:opacity-100 transition-opacity">
                                        </div>
                                        <div>
                                            <h3 class="font-bold text-brand-dark group-hover:text-brand-red transition-colors">
                                                {{ $product->name }}</h3>
                                            <p class="text-sm text-brand-gray mt-1 font-mono font-medium">
                                                ${{ number_format($product->price, 2) }}</p>
                                        </div>
                                        <div class="flex items-center gap-2" onclick="event.stopPropagation();">
                                            <input type="number" name="quantity" value="1" min="1"
                                                class="w-12 text-center rounded-xl border-gray-200 text-sm py-1 focus:ring-brand-gold focus:border-brand-gold"
                                                onclick="event.stopPropagation();">
                                            <div
                                                class="w-10 h-10 rounded-full bg-brand-dark text-brand-gold flex items-center justify-center shadow-md group-hover:bg-brand-red group-hover:text-white transition-colors pointer-events-none">
            ...
            <div id="order-summary-container"
                ...
                <div
                    class="flex-1 lg:overflow-y-auto p-4 space-y-3 custom-scrollbar bg-gray-50/50 min-h-[250px] lg:min-h-0">
                    @foreach($order->items as $item)
                        <div
                            class="bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between animate-fade-in-up">
                            ...
                            <span
                                class="font-mono font-bold text-gray-600 text-sm">${{ number_format($item->subtotal, 2) }}</span>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="bg-white p-5 border-t border-gray-200 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] z-10 space-y-4">
                    <div class="flex justify-between items-end border-b border-gray-100 pb-4">
                        ...
                        <span
                            class="font-serif text-4xl font-bold text-brand-dark">${{ number_format($order->total, 2) }}</span>
                    </div>
                    ...
                    @if($order->status === \App\Enums\OrderStatus::ANOTADO)
                        <form action="{{ route('orders.send-to-kitchen', $order->id) }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="w-full py-4 rounded-xl bg-brand-gold text-brand-dark font-bold text-sm uppercase tracking-wider shadow-lg shadow-brand-gold/30 hover:bg-yellow-400 active:scale-95 transition-all flex flex-col items-center justify-center gap-1">
                    ...
                    @else
                        <div class="bg-gray-100 p-3 rounded-xl text-center border border-gray-200">
                            <p
                                class="text-xs font-bold text-brand-dark uppercase tracking-wide flex items-center justify-center gap-2">
                    ...
                    <button type="button" @click="showModal = true"
                        class="w-full py-4 rounded-xl bg-green-600 text-white font-bold text-sm uppercase tracking-wider shadow-lg shadow-green-600/30 hover:bg-green-700 active:scale-95 transition-all flex items-center justify-center gap-2">
                    ...
                    <div class="relative w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
                        ...
                        <div class="flex-1 overflow-y-auto p-6 bg-gray-50 custom-scrollbar">
                            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-3">
                                @foreach($order->items as $item)
                                    <div
                                        class="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0 last:pb-0">
                            ...
                            <div class="flex justify-between items-center mt-6 px-2">
                                ...
                                <span
                                    class="font-serif text-3xl font-bold text-brand-dark">${{ number_format($order->total, 2) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="p-6 bg-white border-t border-gray-100">
                            <form action="{{ route('orders.checkout', $order->id) }}" method="POST" class="space-y-4">
                                ...
                                <div>
                                    <label
                                        class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Método
                                        de Pago</label>
                                    <div class="relative">
                                        <select name="payment_method"
                                            class="w-full appearance-none bg-gray-50 border border-gray-200 text-brand-dark py-3 px-4 pr-8 rounded-xl leading-tight focus:outline-none focus:bg-white focus:border-brand-gold font-bold">
                                ...
                                <button type="submit"
                                    class="w-full py-4 rounded-xl bg-green-600 text-white font-bold text-sm uppercase tracking-wider shadow-lg hover:bg-green-700 active:scale-95 transition-all">
                                    Confirmar y Cerrar
                                </button>
                            </form>
                        </div>
