...
@section('content')
    <div class="container mx-auto px-4 py-6 font-sans text-brand-dark lg:h-[calc(100vh-80px)] h-auto" x-data="{ 
            items: [],
            addToCart(product) {
        ...
        <div class="flex flex-col lg:flex-row gap-6 lg:h-full pb-10 lg:pb-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($category->products as $product)
                                <div @click="addToCart({ id: {{ $product->id }}, name: '{{ $product->name }}', price: {{ $product->price }} })"
                                    ...
                                    <button type="button"
                                        class="w-10 h-10 rounded-full bg-brand-dark text-brand-gold flex items-center justify-center shadow-md group-hover:bg-brand-red group-hover:text-white transition-colors">
            ...
            <div
                class="w-full lg:w-[400px] flex flex-col bg-white rounded-3xl shadow-xl border border-brand-gold/20 overflow-hidden relative lg:h-full h-auto mt-6 lg:mt-0">
                ...
                <div
                    class="flex-1 lg:overflow-y-auto p-4 space-y-3 custom-scrollbar bg-gray-50/50 min-h-[200px] lg:min-h-0">
                    <template x-if="items.length === 0">
                        <div
                            class="h-full flex flex-col items-center justify-center text-gray-400 opacity-60 py-10 lg:py-0">
                    ...
                    <template x-for="(item, index) in items" :key="item.id">
                        <div
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-brand-gray font-mono mt-0.5">$<span
                            ...
                            <div class="flex items-center gap-3 bg-gray-100 rounded-lg p-1">
                                <button type="button" @click="decrement(index)"
                                    class="w-8 h-8 flex items-center justify-center bg-white rounded-md shadow-sm text-red-500 hover:text-red-700 active:scale-95 transition-all">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <button type="button" @click="increment(index)"
                                    class="w-8 h-8 flex items-center justify-center bg-white rounded-md shadow-sm text-green-600 hover:text-green-800 active:scale-95 transition-all">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                                    </svg>
                ...
                <div
                    class="bg-white p-5 border-t border-gray-200 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] z-10 sticky bottom-0">
                    ...
                    <form action="{{ route('orders.store') }}" method="POST" id="orderForm">
                        ...
                        <button type="submit" :disabled="items.length === 0"
                            class="w-full py-4 rounded-xl bg-brand-red text-white font-bold text-sm uppercase tracking-wider shadow-lg shadow-brand-red/30 hover:bg-[#C81E1E] active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-brand-red disabled:active:scale-100">
