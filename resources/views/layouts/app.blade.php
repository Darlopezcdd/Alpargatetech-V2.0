...
<head>
    ...
    <style>
        ...
        .scrollbar-thin::-webkit-scrollbar-thumb {
            background-color: rgba(217, 191, 162, 0.4);
            border-radius: 20px;
        }

        .scrollbar-thin::-webkit-scrollbar-thumb:hover {
            background-color: rgba(217, 191, 162, 0.7);
        }
...
<body class="h-full bg-gray-50 text-brand-dark" x-data="{ mobileOpen: false, desktopOpen: true }">
    ...
    <div id="toast-container"
        class="fixed top-6 left-1/2 transform -translate-x-1/2 z-[200] flex flex-col gap-3 w-full max-w-md pointer-events-none">
    </div>
    ...
    <div class="min-h-full">
        ...
        <div class="hidden lg:flex lg:flex-col lg:fixed lg:inset-y-0 sidebar-transition border-r border-brand-gold/20 bg-brand-dark overflow-hidden z-[100]"
            ...
            <div class="flex-1 flex flex-col min-h-0">
                <div class="flex-1 flex flex-col overflow-y-auto">
                    <div
                        class="flex items-center justify-between flex-shrink-0 px-4 h-16 border-b border-brand-gold/10">
                ...
                <div
                    class="flex-shrink-0 flex bg-black/30 p-4 border-t border-brand-gold/10 overflow-hidden whitespace-nowrap">
                    <div class="flex-shrink-0 w-full group block">
                        <div class="flex items-center">
                            <div
                                class="inline-block h-9 w-9 rounded-full overflow-hidden bg-brand-gold border-2 border-brand-dark shadow-md flex items-center justify-center shrink-0">
                                <span
                                    class="text-brand-dark font-bold text-lg">{{ substr(Auth::user()->name, 0, 1) }}</span>
                            </div>
                            <div class="ml-3 transition-opacity duration-200"
                                :class="desktopOpen ? 'opacity-100' : 'opacity-0'">
                                <p
                                    class="text-sm font-medium text-white group-hover:text-brand-gold transition-colors truncate w-32">
                                    {{ Auth::user()->name }}</p>
                                <p
                                    class="text-[10px] font-medium text-brand-gold/70 group-hover:text-brand-gold uppercase tracking-wider">
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="mt-3">
                            ...
                            <button type="submit"
                                ...
                                <svg class="mr-2 h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex flex-col flex-1 min-h-screen sidebar-transition" :class="desktopOpen ? 'lg:pl-64' : 'lg:pl-0'">

            <div
                class="sticky top-0 z-[90] flex items-center justify-between h-16 bg-white shadow-sm px-4 sm:px-6 lg:px-8 border-b border-brand-gold/10">
                <div class="flex items-center">
